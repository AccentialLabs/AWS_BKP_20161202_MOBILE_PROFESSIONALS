...
function verificaQtdAgendamentos(){
    var Query = "SELECT count(*) as QtdServicosPendentes FROM schedules INNER JOIN users User ON User.id = schedules.user_id INNER JOIN services Service ON schedules.service_id = Service.id INNER JOIN companies Company ON schedules.companie_id = Company.id INNER JOIN facebook_profiles FbProfile ON FbProfile.user_id = User.id WHERE schedules.status = 1 and secondary_user_id = "+$.cookieStorage.get('SecondaryUser').id+" and DATE(schedules.date) < DATE(NOW()) or schedules.status = 1 and secondary_user_id = "+$.cookieStorage.get('SecondaryUser').id+" and DATE(schedules.date) = DATE(NOW()) and TIME(schedules.time_end) <= TIME(NOW());";
...
$(document).ready(function () {
   ...
   // createNotificationsSchedules();
	...
	var color = {
		companyname:  $.cookieStorage.get('companies').fancy_name,
		username: $.cookieStorage.get('SecondaryUser').name,
		usertype:"professional",
		schedule_solicitation_id:"",
		user_id:$.cookieStorage.get('SecondaryUser').id
	};
		
     $.ajax({
        method: "POST",
        url: "https://"+ip+"/jezzy-colorimetria/Functions.php",
        data: color
    }).done(function(result) {
		
      window.location.href = "https://"+ip+"/jezzy-colorimetria/Home.php";
...
function Schedules() {
    ...
    var queryMENSAL = "SELECT (SELECT  count(*) FROM schedules inner join secondary_users on secondary_users.id = schedules.secondary_user_id WHERE  schedules.companie_id = "+$.cookieStorage.get('companies').id+" and MONTH(schedules.date) = "+((month/1)+1)+" and schedules.status!= 2 and secondary_users.id = "+$.cookieStorage.get('SecondaryUser').id+") AS total_mensal,(SELECT  count(*) FROM schedules inner join secondary_users on secondary_users.id = schedules.secondary_user_id WHERE schedules.status!= 2 and schedules.status!= 0  and schedules.companie_id = "+$.cookieStorage.get('companies').id+" and secondary_users.id = "+$.cookieStorage.get('SecondaryUser').id+" and MONTH(schedules.date) = "+((month/1)+1)+"  AND DAYOFMONTH(schedules.date) >= "+day+") AS total_passado,(SELECT  count(*) FROM schedules inner join secondary_users on secondary_users.id = schedules.secondary_user_id WHERE schedules.status!= 2 and schedules.status!= 0 and schedules.companie_id = "+$.cookieStorage.get('companies').id+" and secondary_users.id = "+$.cookieStorage.get('SecondaryUser').id+" and MONTH(schedules.date) = "+((month/1)+1)+" AND YEARWEEK(schedules.date)=YEARWEEK(NOW())) AS total_semanal;";
...
function qtdVendasMes(){
    ...
    $.ajax({
        ...
        if(result != ''){
            var objReturn = JSON.parse(Base64.decode(result));
				if((objReturn[0][0].sum) != null){
            $("#qtdvendas").html(monetary(objReturn[0][0].sum));
				}else{
					  $("#qtdvendas").html(monetary(0.00));
				}
            RendaMensal();
        }
    ...
    $.ajax({
        ...
        if(result != ''){
			...
			if((objReturn[0][0].sum) != null){
				...
				$("#valorcomissao").html(monetary(0.00));
			}
...
function CarregaAgendamentos(){
    ...
    var day = date.date();
			// AGENDAMENTOS CONFIRMADOS
			var Query = 'SELECT Schedules.date, Schedules.id, Schedules.user_id, Schedules.time_begin, Schedules.time_end, Schedules.status, Schedules.subclasse_name, Schedules.voucher_id, User.name, User.gender, User.photo, User.birthday, FbProfile.chemistry, FbProfile.hair_type, Company.fancy_name, Service.subclasse_id FROM schedules Schedules INNER JOIN users User ON User.id = Schedules.user_id INNER JOIN companies Company ON Schedules.companie_id = Company.id INNER JOIN facebook_profiles FbProfile ON FbProfile.user_id = User.id INNER JOIN services Service ON Schedules.service_id = Service.id WHERE MONTH(Schedules.date) = '+((month/1)+1)+' and DAYOFMONTH(Schedules.date) = ' + day + ' and Schedules.status = 1 and Schedules.secondary_user_id = ' + $.cookieStorage.get('SecondaryUser').id+' UNION ALL SELECT SchedulesSolicitation.date, SchedulesSolicitation.id, SchedulesSolicitation.user_id, SchedulesSolicitation.time_begin, SchedulesSolicitation.time_end, SchedulesSolicitation.status, SchedulesSolicitation.service_name, SchedulesSolicitation.voucher_id, User.name, User.gender, User.photo, User.birthday, FbProfile.chemistry, FbProfile.hair_type, Company.fancy_name, Service.subclasse_id FROM schedules_solicitation SchedulesSolicitation INNER JOIN users User ON User.id = SchedulesSolicitation.user_id INNER JOIN companies Company ON SchedulesSolicitation.company_id = Company.id INNER JOIN facebook_profiles FbProfile ON FbProfile.user_id = User.id INNER JOIN services Service ON SchedulesSolicitation.service_id = Service.id WHERE MONTH(SchedulesSolicitation.date) = '+((month/1)+1)+' and DAYOFMONTH(SchedulesSolicitation.date) =  ' + day + ' and SchedulesSolicitation.secundary_user_id = ' + $.cookieStorage.get('SecondaryUser').id+' and SchedulesSolicitation.status = "WAITING_COMPANY_RESPONSE" and TIME(SchedulesSolicitation.time_end) >= TIME(NOW()) and TIME(SchedulesSolicitation.time_begin) >= TIME(NOW()) ORDER BY status DESC, time_begin ASC;';
                        ...
                        if(result != ''){
							...
							for(var i=0;i<objReturn.length;i++){
								...
								var have = false;
								 ...
								 var data =  moment(allkindschedule[0].date+" "+allkindschedule[0].time_begin);
                               // console.log(data);
				...
				var empresa = allkindschedule[0].fancy_name;
					...
					var Query = 'SELECT * FROM schedules_solicitation_parameters SSParameters INNER JOIN schedules_solicitation SS ON SSParameters.schedules_solicitation_id = SS.id WHERE SS.user_id ='+allkindschedule[0].user_id+' and SS.secundary_user_id = '+$.cookieStorage.get("SecondaryUser").id+' and DAYOFMONTH(SS.date) =  ' + day + '  and SS.time_begin = "'+allkindschedule[0].time_begin+'";';
			;
                    var conditions = {
                        'User':{
                            'query':Query
					...
					horarios +=""+dia+"/"+mes+"/"+ano+","+allkindschedule[0].time_begin+",";
				}else{
					horarios +=""+dia+"/"+mes+"/"+ano+","+allkindschedule[0].time_begin+"";
				}
                  ...
                  // calcular idade do usuario
                        ...
                        if (mes_atual < mes_aniversario || mes_atual == mes_aniversario && dia_atual < dia_aniversario) {
                            quantos_anos--;
                                    ...
                                    //console.log(moment(horafinaldogendamento));
                                    ...
                                    if(japassou == true){
                                        ...
                                        if(allkindschedule[0].voucher_id==0){
											...
											$("#swipeactionright").append('<div class="col-xs-12" style="border:1px solid lightgrey;background-color: #78BDCA;height: 15vw!important;color:white;font-family: \'Open Sans\';padding-top:2vw;margin-left:0%;margin-right:0%;padding-left:0%;padding-right:0%;"><div class="col-xs-9"><span class="col-xs-2">' +  dia + '/' + str_month + '</span><span class="col-xs-10">'+allkindschedule[0].name+'</span><span class="col-xs-2">' + hora +':'+str_minutos + '</span><span style="overflow: hidden;white-space: nowrap;" class="col-xs-10">'+(allkindschedule[0].subclasse_name)+'</span></div><div class="col-xs-3" style="margin-top:1%;font-size: 7vw;font-weight: 100;"><img id="changesignal'+allkindschedule[0].id+'"  src="img/icons/mais.png" onclick="expand('+allkindschedule[0].id+',\''+serviceinteger+'\', 1, \''+hora +':'+str_minutos+'\');" style="height:30px;width:30px;align-items: center;margin-left: 30px;"></div></div><div id="'+allkindschedule[0].id+'" class="col-xs-12 hide" style="color:white;font-family: \'Open Sans\';margin-left:0%;margin-right:0%;"><div class="col-xs-12" style="color:#2597AC;padding:2%;"><div class="col-xs-3" style="padding:2%;"><img style="object-fit:cover;height:20vw!important;border-radius:20vh;width:12vh;border: 1px solid #2597AC;" class="fotousuario profile-pic" src = '+pic+'></div><div class="col-xs-9"><div class="col-xs-12" style="padding:2%;font-weight:bolder;"">'+allkindschedule[0].name.toUpperCase()+'</div><div class="col-xs-12" style="padding:1%;"><div class="col-xs-4" style="font-weight:bolder;">SEXO</div><div class="col-xs-8">Feminino</div></div><div class="col-xs-12"  style="padding:1%;"><div class="col-xs-4" style="font-weight:bolder;">IDADE</div><div class="col-xs-8">'+quantos_anos+' anos</div></div><div class="col-xs-12"  style="padding:1%;"><div class="col-xs-4" style="font-weight:bolder;">CABELO</div><div class="col-xs-8">'+cabelo+'</div></div></div></div><div class="col-xs-12" style="color:#2597AC;text-align:center;padding:2%;font-weight:bolder;border-top:1px solid #2597AC;">'+allkindschedule[0].subclasse_name.toUpperCase()+'</div><div class="col-xs-12" style="color:#2597AC;"><div class="col-xs-9" style="padding:0%;margin:0%;padding-left:2%;"><div class="col-xs-3" style="padding:0%;margin:0%;z-index:999999;"><img style="width: 100%;border-radius: 50%;border:1px solid #2597AC;"src='+OriginalColorImg+'><br><div style="width: 100%;text-align: center;">'+OriginalColor+'</div></div><div class="col-xs-3" style="padding:0%;margin:0%;margin-left:-5%;"><img style="width: 100%;border-radius: 50%;border:1px solid #2597AC;"src='+HairLengthImg+'><br><div style="width: 100%;text-align: center;">'+HairLength+'</div></div><div class="col-xs-3" style="padding:0%;margin:0%;z-index:-1;margin-left:-5%;"><img style="width: 100%;border-radius: 50%;"src='+WhiteHairPercentageImg+'><br><div style="width: 100%;text-align: center;">'+WhiteHairPercentage+'%</div></div><div class="col-xs-3" style="padding:0%;margin:0%;"><img src="img/icons/ChevronTurquesaDireita.png" style="margin-top: 25%;    width: 40%;margin-left: 50%;"></div></div><div class="col-xs-3" style="padding: 0%;"><img style="width: 70%;border-radius: 50%;border:1px solid #2597AC;"src='+NuanceImg+'><br><div style="width: 70%;text-align: center;">'+Nuance+'</div></div></div></div></div>'); 
									 ...
									 if(japassou == true){
                                        ...
                                        if(allkindschedule[0].voucher_id==0){
											...
											$("#swipeactionright").append('<div class="col-xs-12" style="border:1px solid lightgrey;background-color: #78BDCA;height: 15vw!important;color:white;font-family: \'Open Sans\';padding-top:2vw;margin-left:0%;margin-right:0%;padding-left:0%;padding-right:0%;"><div class="col-xs-9"><span class="col-xs-2">' +  dia + '/' + str_month + '</span><span class="col-xs-10">'+allkindschedule[0].name+'</span><span class="col-xs-2">' + hora +':'+str_minutos + '</span><span style="overflow: hidden;white-space: nowrap;" class="col-xs-10">'+(allkindschedule[0].subclasse_name)+'</span></div><div class="col-xs-3" style="margin-top:1%;font-size: 7vw;font-weight: 100;"><img id="changesignal'+allkindschedule[0].id+'"  src="img/icons/mais.png" onclick="expand('+allkindschedule[0].id+',\''+serviceinteger+'\', 1, \''+hora +':'+str_minutos+'\');" style="height:30px;width:30px;align-items: center;margin-left: 30px;"></div></div><div id="'+allkindschedule[0].id+'" class="col-xs-12 hide" style="color:white;font-family: \'Open Sans\';margin-left:0%;margin-right:0%;"><div class="col-xs-12" style="color:#2597AC;padding:2%;"><div class="col-xs-3" style="padding:2%;"><img style="object-fit:cover;height:20vw!important;border-radius:20vh;width:12vh;border: 1px solid #2597AC;" class="fotousuario profile-pic" src = '+pic+'></div><div class="col-xs-9"><div class="col-xs-12" style="padding:2%;font-weight:bolder;"">'+allkindschedule[0].name.toUpperCase()+'</div><div class="col-xs-12" style="padding:1%;"><div class="col-xs-4" style="font-weight:bolder;">SEXO</div><div class="col-xs-8">Feminino</div></div><div class="col-xs-12"  style="padding:1%;"><div class="col-xs-4" style="font-weight:bolder;">IDADE</div><div class="col-xs-8">'+quantos_anos+' anos</div></div><div class="col-xs-12"  style="padding:1%;"><div class="col-xs-4" style="font-weight:bolder;">CABELO</div><div class="col-xs-8">'+cabelo+'</div></div></div></div></div>'); 
							...
							//AGENDAMENTOS SOLICITADOS
								...
								if(allkindschedule[0].subclasse_id >=15 && allkindschedule[0].subclasse_id <=23 && have == true){ //verifica se é agendamento de coloração para preparar visualização dos parametros
									$("#swipeactionright").append('<div class="col-xs-12" style="background-color: #2597AC;border:1px solid lightgrey;height: 15vw!important;color:white;font-family: \'Open Sans\';padding-top:2vw;margin-left:0%;margin-right:0%;padding-left:0%;padding-right:0%;"><div class="col-xs-8" style="padding-left: 4%!important";><span class="col-xs-2">'+date+'</span><span class="col-xs-10" style="    padding-left: 8%;">'+allkindschedule[0].name+'</span><span class="col-xs-2">'+hour+'</span><span style="overflow: hidden;white-space: nowrap;padding-left: 8%;" class="col-xs-10">'+allkindschedule[0].subclasse_name+'</span></div><div class="col-xs-4" style="font-size: 7vw;font-weight: 100;padding:0%;"><div class="col-xs-12" style="padding:0%;"><img class="col-xs-4" onclick="infocoloracao(\''+Nuance+'\', \''+WhiteHairPercentage+'\', \''+OriginalColor+'\', \''+HairLength+'\')" src="img/icons/mais.png" style="height:35px;width:35px;padding-top:5%!important;padding-left: 2%!important;padding-right: 2%!important; padding-bottom: 2%!important;"><img class="col-xs-4" src="img/icons/ok%20-%20check.png" onclick="aceptSchedule('+allkindschedule[0].id+', \''+utf8_decode(allkindschedule[0].subclasse_name)+'\', \''+allkindschedule[0].name+'\', \''+hora+':'+str_minutos+'\', \'' +  dia + '/' + str_month + '\')" style="height:40px;width:40px;padding: 8%!important;padding-top:7%!important;"><img class="col-xs-4" src="img/icons/Sair%20-%20branco-07.png" onclick="recuseSchedule('+allkindschedule[0].id+', \''+utf8_decode(allkindschedule[0].subclasse_name)+'\', \''+allkindschedule[0].name+'\', \''+hora+':'+str_minutos+'\', \'' +  dia + '/' + str_month + '\')" style="height:30px;width:25px;padding-top:7%!important;padding-left:2%!important;"></div></div></div>');
								}else{
									$("#swipeactionright").append('<div class="col-xs-12" style="background-color: #2597AC;border:1px solid lightgrey;height: 15vw!important;color:white;font-family: \'Open Sans\';padding-top:2vw;margin-left:0%;margin-right:0%;padding-left:0%;padding-right:0%;"><div class="col-xs-9"><span class="col-xs-2">'+date+'</span><span class="col-xs-10">'+allkindschedule[0].name+'</span><span class="col-xs-2">'+hour+'</span><span style="overflow: hidden;white-space: nowrap;" class="col-xs-10">'+allkindschedule[0].subclasse_name+'</span></div><div class="col-xs-3" style="margin-top:2.5%;font-size: 7vw;font-weight: 100;"><img src="img/icons/ok%20-%20check.png" onclick="aceptSchedule('+allkindschedule[0].id+', \''+utf8_decode(allkindschedule[0].subclasse_name)+'\', \''+allkindschedule[0].name+'\', \''+hora+':'+str_minutos+'\', \'' +  dia + '/' + str_month + '\')" style="height:22px;width:22px;"><img src="img/icons/Sair%20-%20branco-07.png" onclick="recuseSchedule('+allkindschedule[0].id+', \''+utf8_decode(allkindschedule[0].subclasse_name)+'\', \''+allkindschedule[0].name+'\', \''+hora+':'+str_minutos+'\', \'' +  dia + '/' + str_month + '\')" style="height:22px;width:22px;float:right;"></div></div>');
								}
							}
