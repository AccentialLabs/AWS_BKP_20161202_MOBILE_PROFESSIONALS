...
/*function SimularClique(){
        ...
        var botaofacebook =  document.getElementById("facebook");

        botaofacebook.click();

}*/
function sendRequest() {
 var query = 'SELECT * FROM secondary_users SecondaryUser INNER JOIN companies ON companies.id = SecondaryUser.company_id WHERE SecondaryUser.email = "'+ $("#nameInput").val() + '" and SecondaryUser.password = "' + md5($("#passInput").val()).toString() + '" and SecondaryUser.excluded  = 0';
    var conditions = {
            'User': {
            'query': query
        }
    ...
    $.ajax({
        ...
        var convertedReturn =(JSON.parse(decodeObjReturn));
            ...
            if ($.cookieStorage.isSet('SecondaryUser')) {
                ...
                localStorage.setItem('niverseen', '0');
               console.log(localStorage.getItem('niverseen'));
                window.location.href = 'home.html';
            ...
            $.ajax({
                ...
                if (result != "") {
                    generateModalAlert("Senha Inválida!");
                    $('#mymodal').modal('show');
                } else{
                    generateModalAlert("Usuário Inexistente! Cadastre-se ou verifique usuário");
                    $('#mymodal').modal('show');
            }
            }).error(function(XMLHttpRequest, textStatus, errorThrown) {
                alert(errorThrown);
            ...
            var query = 'SELECT * FROM secondary_users SecondaryUser INNER JOIN companies ON companies.id = SecondaryUser.company_id WHERE SecondaryUser.email = "' + $("#nameInput").val() + '" and SecondaryUser.excluded  = 0';
            var conditions = {
                'User': {
                    'query': query
                }
            ...
            $.ajax({
                ...
                if (result != "") {
                    ...
                    generateModalAlert("Usuário não existe!");
                    $('#mymodal').modal('show');
                }
            }).error(function (XMLHttpRequest, textStatus, errorThrown) {
                alert(errorThrown);
